...
class NPCAtributosFisicosModal(NPCModalBase):
  def __init__(self, npc_context):
    super().__init__(npc_context, title=f"Atributos Físicos (CoC) de {npc_context.npc_name}")
    atributos = self.npc_data.get("atributos", {})
    self.forca = self.create_input("Força (STR)", atributos.get("Força"))
    self.destreza = self.create_input("Destreza (DEX)", atributos.get("Destreza"))
    self.constituicao = self.create_input("Constituição (CON)", atributos.get("Constituição"))
    self.tamanho = self.create_input("Tamanho (SIZ)", atributos.get("Tamanho"))
    self.aparencia = self.create_input("Aparência (APP)", atributos.get("Aparência"))

    self.add_item(self.forca)
    self.add_item(self.destreza)
    self.add_item(self.constituicao)
    self.add_item(self.tamanho)
  ...
  async def on_submit(self, interaction: discord.Interaction):
    try:
      valores = {
        "Força": self.forca.value, "Destreza": self.destreza.value,
        "Constituição": self.constituicao.value, "Tamanho": self.tamanho.value,
