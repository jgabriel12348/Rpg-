...
class NPCOrdemMentaisModal(NPCModalBase):
  def __init__(self, npc_context):
    ...
    self.intelecto = self.create_input("Intelecto (INT)", atributos.get("Intelecto"))
    self.percepcao = self.create_input("Percepção (PER)", atributos.get("Percepção"))
    self.vontade = self.create_input("Vontade (VON)", atributos.get("Vontade"))
  ...
  async def on_submit(self, interaction: discord.Interaction):
    try:
      valores = {
        "Intelecto": self.intelecto.value, "Percepção": self.percepcao.value,
        "Vontade": self.vontade.value
      }
