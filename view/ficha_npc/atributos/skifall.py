...
from models.npc_modals.atributos_modais.skyfall.especiais import NPCSkifallRecursosModal
from models.npc_modals.atributos_modais.skyfall.fisicos import NPCSkifallFisicosModal
from models.npc_modals.atributos_modais.skyfall.mentais import NPCSkifallMentaisModal

class NPCSkifallAtributosView(discord.ui.View):
  ...
  async def fisicos(self, interaction: discord.Interaction, button: discord.ui.Button):
    await interaction.response.send_modal(NPCSkifallFisicosModal(self.npc_context))
  ...
  async def mentais(self, interaction: discord.Interaction, button: discord.ui.Button):
    await interaction.response.send_modal(NPCSkifallMentaisModal(self.npc_context))

  @discord.ui.button(label="✨ Especiais", style=discord.ButtonStyle.success)
  async def especiais(self, interaction: discord.Interaction, button: discord.ui.Button):
    await interaction.response.send_modal(NPCSkifallRecursosModal(self.npc_context))
